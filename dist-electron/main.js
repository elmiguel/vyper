      ...
      addFields(fieldDescriptions) {
        ...
        for (let i = 0; i < fieldDescriptions.length; i++) {
          ...
          if (this._types) {
            ...
            this._parsers[i] = types4.getTypeParser(desc2.dataTypeID, desc2.format || "text");
          }
        }
...
var appState = pgTable("app_state", {
...
var scenes = pgTable("scenes", {
  ...
  gameCamera: jsonb("game_camera").$type().notNull().default({}),
...
var scripts = pgTable("scripts", {
  ...
  gameId: uuid("game_id").notNull().references(() => games.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  mode: text("mode").notNull().default("nodes"),
  // 'nodes' | 'code'
  code: text("code").notNull().default(""),
  codeDirty: boolean("code_dirty").notNull().default(false),
  enabled: boolean("enabled").notNull().default(true),
  // { nodes, edges } React Flow graph.
  graph: jsonb("graph").$type().notNull().default({}),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow()
});
var sceneVersions = pgTable("scene_versions", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  gameCamera: jsonb("game_camera").$type().notNull().default({}),
  gridVisible: boolean("grid_visible").notNull().default(true),
  scripts: jsonb("scripts").$type().notNull().default([]),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow()
});
...
// electron/db/embedded.ts
async function openEmbedded(dataDir) {
  const pglite = new PGlite2(dataDir);
  await pglite.waitReady;
  await ensureSchema((sql2) => pglite.exec(sql2));
  const db = drizzle(pglite, { schema: schema_exports });
  return { pglite, db };
}
...
function createDataService(db) {
  const touchGame = (id) => db.update(games).set({ updatedAt: /* @__PURE__ */ new Date() }).where(eq(games.id, id));
  return {
    // ----- App state (singleton) -----
    async getApp() {
    ...
    async putApp(body) {
      const [row] = await db.update(appState).set({ lastGameId: body?.lastGameId ?? null, data: body?.data ?? {}, updatedAt: /* @__PURE__ */ new Date() }).where(eq(appState.id, "singleton")).returning();
      return row;
    },
    // ----- Games -----
    async listGames() {
      const rows = await db.select().from(games).orderBy(desc(games.updatedAt));
      const counts = await db.select({ gameId: scenes.gameId, n: sql`count(*)::int` }).from(scenes).groupBy(scenes.gameId);
    ...
    async getGame(id) {
      ...
      }).from(scenes).where(eq(scenes.gameId, game.id)).orderBy(scenes.orderIndex);
    ...
    async createGame(name, description = "") {
      ...
      const [scene] = await db.insert(scenes).values({ gameId: game.id, name: "Scene 1", orderIndex: 0 }).returning();
      const [updated] = await db.update(games).set({ activeSceneId: scene.id }).where(eq(games.id, game.id)).returning();
      return { game: updated, scenes: [scene], scripts: [] };
    },
    async patchGame(id, patch) {
      const set = { updatedAt: /* @__PURE__ */ new Date() };
      if (patch?.name !== void 0) set.name = String(patch.name).slice(0, 120);
      if (patch?.description !== void 0) set.description = String(patch.description);
      if (patch?.activeSceneId !== void 0) set.activeSceneId = patch.activeSceneId;
      if (patch?.settings !== void 0) set.settings = patch.settings;
    ...
    async putScripts(gameId, incoming) {
      ...
      await db.transaction(async (tx) => {
        ...
        for (const s of list) {
          const values = {
            ...
            enabled: s.enabled === void 0 ? true : Boolean(s.enabled),
            graph: s.graph ?? {},
    ...
    async createScene(gameId, name) {
      const [{ max }] = await db.select({ max: sql`coalesce(max(${scenes.orderIndex}), -1)::int` }).from(scenes).where(eq(scenes.gameId, gameId));
      const nm = String(name ?? `Scene ${(max ?? -1) + 2}`);
      const [scene] = await db.insert(scenes).values({ gameId, name: nm, orderIndex: (max ?? -1) + 1, entities: [] }).returning();
      await touchGame(gameId);
      return scene;
    },
    async patchScene(id, patch) {
      const set = { updatedAt: /* @__PURE__ */ new Date() };
      if (patch?.name !== void 0) set.name = String(patch.name);
      if (patch?.entities !== void 0) set.entities = patch.entities;
      if (patch?.gameCamera !== void 0) set.gameCamera = patch.gameCamera;
      if (patch?.gridVisible !== void 0) set.gridVisible = Boolean(patch.gridVisible);
      if (patch?.orderIndex !== void 0) set.orderIndex = Number(patch.orderIndex);
    ...
    async deleteScene(id) {
      ...
      if (game?.activeSceneId === scene.id) {
        ...
        await db.update(games).set({ activeSceneId: next.id, updatedAt: /* @__PURE__ */ new Date() }).where(eq(games.id, game.id));
      } else {
        await touchGame(scene.gameId);
      }
    },
    // ----- Scene versions (revert history) -----
    async createVersion(sceneId, body) {
      const [scene] = await db.select().from(scenes).where(eq(scenes.id, sceneId));
      if (!scene) throw new ServiceError(404, "scene not found");
      const isManual = body?.kind === "manual";
      const [row] = await db.insert(sceneVersions).values({
        sceneId,
        gameId: scene.gameId,
        label: String(body?.label ?? ""),
        kind: isManual ? "manual" : "auto",
        entities: body?.entities ?? scene.entities,
        gameCamera: body?.gameCamera ?? {},
        gridVisible: body?.gridVisible === void 0 ? true : Boolean(body.gridVisible),
    ...
    async listVersions(sceneId) {
      ...
      }).from(sceneVersions).where(eq(sceneVersions.sceneId, sceneId)).orderBy(desc(sceneVersions.createdAt)).limit(100);
    },
    async getVersion(id) {
...
function registerIpc(svc) {
  ipcMain.handle("vyper:invoke", async (_event, method, args) => {
    const fn = svc[method];
...
// electron/sync/postgres.ts
async function connectPostgres(url, timeoutMs = 2500) {
  if (!url) return null;
  const pool = new esm_default.Pool({ connectionString: url, connectionTimeoutMillis: timeoutMs, max: 2 });
  try {
    await pool.query("select 1");
    return { db: drizzle2(pool, { schema: schema_exports }), dispose: () => pool.end() };
  } catch {
...
// electron/sync/merge.ts
function mergeByTimestamp(local, remote, opts = {}) {
  ...
  for (const l of local) {
    ...
    const lt2 = toMillis(l[ts]);
    const rt = toMillis(r[ts]);
    if (lt2 > rt) toRemote.push(l);
    else if (rt > lt2) toLocal.push(r);
  }
  for (const r of remote) {
    if (!seen.has(String(r[key]))) toLocal.push(r);
  }
  return { toLocal, toRemote };
}
function unionById(local, remote, key = "id") {
...
// electron/sync/syncTables.ts
function selectAll(db, table) {
  return db.select().from(table);
}
async function upsertRows(db, table, pk, rows) {
  if (!rows.length) return;
  const set = {};
  for (const [field, col] of Object.entries(getTableColumns(table))) {
    set[field] = sql.raw(`excluded.${col.name}`);
  }
...
function startSync(embedded, databaseUrl) {
  let stopped = false;
  let wasReachable = null;
  const tick = async () => {
    if (stopped) return;
    try {
      ...
      if (synced !== wasReachable) {
        console.log(
          synced ? "[vyper] Postgres reachable \u2014 synced embedded \u2194 server (newest wins)." : "[vyper] Postgres not reachable \u2014 using the embedded database."
        );
        wasReachable = synced;
      }
    } catch (err) {
...
async function runOnce(embedded, databaseUrl) {
  const remote = await connectPostgres(databaseUrl);
  if (!remote) return false;
  const local = embedded.db;
  const rdb = remote.db;
  try {
...
async function reconcileByTs(local, remote, table, pk) {
...
async function reconcileUnion(local, remote, table, pk) {
  const [l, r] = await Promise.all([selectAll(local, table), selectAll(remote, table)]);
...
var RENDERER_DIR = path.join(__dirname, "../dist");
...
var MIME = {
  ...
  // required: Havok loads via WebAssembly.instantiateStreaming
  ".svg": "image/svg+xml",
  ".png": "image/png",
  ".jpg": "image/jpeg",
  ".ico": "image/x-icon",
...
async function loadDevUrl(win, url, attempts = 20) {

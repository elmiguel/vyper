...
/**
 * Generate box / tri-planar UVs for a kernel mesh that has none (the half-edge kernel doesn't
 * produce UVs). Each vertex is projected onto the world plane perpendicular to the dominant
 * axis of its normal, so axis-aligned faces (cubes, planes, extrusions) get sensible,
 * world-scaled texture mapping for the Studio's PBR preview. `scale` is texture tiles per world
 * unit. Not a true unwrap — enough to preview materials with visible detail.
 */
 ...
 * that heuristic misses edges on a flat-shaded, triangulated mesh, so some faces showed no
 * wireframe. Returns undefined when the geometry carries no polygon topology.
 */
